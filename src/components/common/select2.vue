<template>
  <div class="dropdown-container">
    ...
    <ul class="list-module" v-if="isShowData" :class="isShowNone?'none':''">
      <li v-for="(item,index) in datalist" @click="appClick(item)" :key="index" class="list-module-li">
        <span class="list-item-text">{{item}}</span>
      </li>
    </ul>
    <ul class="list-module" v-show="isShowNone">
      <li class="list-module-li">
...
export default {
  ...
  mounted() {
    ...
    document.addEventListener('click',(e)=>{
      ...
      if(e.target.className == 'list-module-li' || e.target.className == 'el-icon-lx-search'){
        // this.$emit('showSelect',true)
